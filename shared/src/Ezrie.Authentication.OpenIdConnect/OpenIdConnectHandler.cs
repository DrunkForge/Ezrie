...
using Ezrie.Authentication;
using Ezrie.Authentication.Events;
...
public class OpenIdConnectHandler : RemoteAuthenticationHandler<OpenIdConnectOptions>, IAuthenticationSignOutHandler
{
	...
	/// <inheritdoc />
	protected virtual async Task<Boolean> HandleRemoteSignOutAsync()
	{
	...
	protected virtual async Task<Boolean> HandleSignOutCallbackAsync()
	{
	...
	protected virtual async Task<OpenIdConnectMessage> RedeemAuthorizationCodeAsync(OpenIdConnectMessage tokenEndpointRequest)
	{
		...
		{
			Logger.LogDebug($"Unexpected token response format. Status Code: {(Int32)responseMessage.StatusCode}. Content-Type header is missing.");
		}
		else if (!String.Equals(contentMediaType, "application/json", StringComparison.OrdinalIgnoreCase))
		{
			Logger.LogDebug($"Unexpected token response format. Status Code: {(Int32)responseMessage.StatusCode}. Content-Type {responseMessage.Content.Headers.ContentType}.");
		}
		...
		{
			throw new OpenIdConnectProtocolException($"Failed to parse token response body as JSON. Status Code: {(Int32)responseMessage.StatusCode}. Content-Type: {responseMessage.Content.Headers.ContentType}", ex);
		}
	...
	private void SaveTokens(AuthenticationProperties properties, OpenIdConnectMessage message)
	{
		...
		{
			if (Int32.TryParse(message.ExpiresIn, NumberStyles.Integer, CultureInfo.InvariantCulture, out var value))
			{
	...
	private OpenIdConnectProtocolException CreateOpenIdConnectProtocolException(OpenIdConnectMessage message, HttpResponseMessage? response)
	{
		...
		if (response != null)
		{
			Logger.ResponseErrorWithStatusCode(message.Error, description, errorUri, (Int32)response.StatusCode);
		}
		else
		{
